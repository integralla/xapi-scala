...
/** Trait to be extended by models that provide property validation
  *
  * On initialization of a class that extends this trait, validation will be
  * performed automatically and an `StatementValidationException` will be thrown
  * if any validation failures are detected.
  */
trait StatementValidation {

  /** Abstract method to be implemented by extending classes which performs one
    * or more validations and returns the results as a sequence which will be
    * further processed by this trait
    *
    * @return
  ...
  /** As part of the trait's constructor, this code will be run on
    * initialization of any extending class, throwing an exception if any
