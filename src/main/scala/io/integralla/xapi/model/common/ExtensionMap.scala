...
/** Extensions are defined by a map and logically relate to the part of the Statement where they are
  ...
  *
  * @param value
  *   A map where each key must be a valid IRI, and each value a JSON data structure or value
  */
case class ExtensionMap(value: Map[IRI, Json]) extends Equivalence {

  /** Generates a signature that can be used to test logical equivalence between objects
    *
    * For extensions, the signature is computed first by concatenating the signature of each IRI
    * with a string representation of the JSON data structure/value, sorted by keys and with no
    * spaces. Then, that resulting list of string values is sorted and hashed as usual
    *
    * @return
    *   A string identifier
    */
