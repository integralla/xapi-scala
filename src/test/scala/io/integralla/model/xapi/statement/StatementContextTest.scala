...
class StatementContextTest extends UnitSpec {
  ...
  val sampleInstructor: Agent = Agent(Some(StatementObjectType.Agent), Some("Andrew Downes"), None, None, None, Some(Account("http://www.example.com", "13936749")))
  val sampleTeam: Group = Group(StatementObjectType.Group, Some("Team PB"), Some(MBox("mailto:teampb@example.com")), None, None, None, None)
  ...
  val sampleContextActivities: ContextActivities = ContextActivities(
    Some(sampleParent), None, Some(sampleCategory), Some(sampleOther)
  )

  val sampleStatementRef: StatementRef = StatementRef(StatementObjectType.StatementRef, UUID.fromString("6690e6c9-3ef0-4ed3-8b37-7f3964730bee"))

  val sampleExtensionValue: Json = Map("name" -> "Kilby", "id" -> "http://example.com/rooms/342").asJson
  val sampleExtensions: Extensions = Map(IRI("http://example.com/profiles/meetings/activitydefinitionextensions/room") -> sampleExtensionValue)
